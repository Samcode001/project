...
const lerp = (start: number, end: number) => {
  // Smooths the movement from one value to another, progressing only 3% of the way each frame. Creates a smooth, gliding camera rather than a jerky hard cut.
...
const Camera = ({
  ...
  const cameraPosition = useRef<{ x: number; y: number }>({
    // The current pixel location of the camera's top-left (starts centered).
  ...
  useTick(() => {
    if (containerRef.current) {
      ...
      const targetX =
        canvasSize.width / 2 -
        heroPosition.x * TILE_SIZE * ZOOM +
        TILE_SIZE * 12;
      const targetY =
        canvasSize.height / 2 -
        heroPosition.y * TILE_SIZE * ZOOM +
        TILE_SIZE * 4;

      // targetValues = { targetX, targetY };
